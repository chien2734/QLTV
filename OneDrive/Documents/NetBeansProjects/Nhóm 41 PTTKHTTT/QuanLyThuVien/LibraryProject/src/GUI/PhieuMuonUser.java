...
public class PhieuMuonUser extends JPanel{
    ...
    private JButton jb_giahan, jb_timkiem, jb_xemtatca;
    ...
    public PhieuMuonUser(String tenDN) {
        System.out.println("Mã độc giả " + tenDN);
        this.tenDN = tenDN;
        this.init();
    ...
    private void init() {
        ...
        String[] header_pm = new String[]{"Mã phiếu", "Ngày đăng ký", "Ngày mượn", "Hạn trả", "Tiền cọc", "Trạng thái"};
        ...
        if (themuon != null) {
            ArrayList<PhieuMuonDTO> dspm = pmBLL.getBorrowByCondition("MaTheMuon = '" + themuon.getMaTheMuon() + "' and TrangThai <> N'Xoá'");
            for (PhieuMuonDTO pm : dspm) {
                System.out.println(pm);
                if (pm.getNgayDK()== null && pm.getHanNhan()== null) {
                    tblModel_pm.addRow(new Object[]{pm.getMaPhieu(), "-", pm.getNgayMuon(), pm.getHanTra(), pm.getTienCoc(), pm.getTrangThai()});
                } else if (pm.getNgayMuon()== null && pm.getHanTra()== null) {
                    tblModel_pm.addRow(new Object[]{pm.getMaPhieu(), pm.getNgayDK(), "-", "-", pm.getTienCoc(), pm.getTrangThai()});
                }
            }
        }
        ...
        //--------------------------------------
        jb_giahan = new JButton("Gia hạn phiếu mượn");      // Gia hạn phiếu có trạng thái = "Đã mượn" trước khi hết hạn
        jb_giahan.setBounds(650, 880, 350, 40);
            ...
            try {
                ...
                for (PhieuMuonDTO pm : list) {
                    if (pm.getNgayDK()== null && pm.getHanNhan()== null) {
                        ...
                        tblModel_pm.addRow(new Object[]{pm.getMaPhieu(), pm.getNgayDK(), pm.getHanNhan(), "-", "-", pm.getTienCoc(), pm.getTrangThai()});
                    }
                }
            } catch (Exception e) {
...
//                java.sql.Date ngayMuon = java.sql.Date.valueOf(ngaymuon);
                ...
                if (ngay > 19) {
                    ...
                    if (check != 0) {
                        tb_pm.setValueAt(hantra, selected, 3);
